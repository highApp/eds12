...
class EditComments extends StatefulWidget {
  ...
  EditComments({required this.comment, required this.onAddCommentSuccess});
  @override
...
class _EditCommentsState extends State<EditComments> {
  ...
  void initState() {
    super.initState();
    txtDateController.text = widget.comment.date;
    txtCommentsController.text = widget.comment.comment;
  }
  ...
  _btnActionEditComment(BuildContext context) {
    ...
    if (txtDateController.text == '') {
      ...
      _callAPIEditComments(context);
    }
  }

  _callAPIEditComments(BuildContext context) {
    this.setState(() {
      isLoading = true;
    });
    ...
    ApiManager networkCal =
        ApiManager(APIConstants.editComment, body, false, header);
